...
const useVideoCall = (socket, authUser, selectedUser) => {
    ...
    const iceCandidatesQueue = useRef([]); // This is the key improvement from the second codebase
    ...
    const acceptCall = async () => {
        try {
            ...
            // Process queued ICE candidates after setting remote description (improvement from second codebase)
    ...
    const endCall = () => {
        ...
        // Ensure 'to' is the correct recipient for ending the call (either selectedUser or incomingCall.from)
        const recipientId = selectedUser ? selectedUser._id : (incomingCall ? incomingCall.from : null);
        if (recipientId) {
            socket.emit('endCall', {
                to: recipientId,
                callId: callId
    ...
    const cleanupCall = () => {
        ...
        iceCandidatesQueue.current = []; // Clear queued candidates on cleanup
    ...
    useEffect(() => {
        ...
        const handleCallAccepted = async ({ answer, callId }) => {
            ...
            if (peerRef.current) {
                ...
                // Process any queued ICE candidates after setting remote description (improvement from second codebase)
        ...
        const handleIceCandidate = async ({ candidate, from, callId }) => {
            ...
            // This logic is directly from the second codebase's improved ICE handling
            const iceCandidate = new RTCIceCandidate(candidate);
            if (peerRef.current?.remoteDescription?.type) { // Check if remote description is set
                try {
